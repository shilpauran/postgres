...
az_query_recovery="psql -h ${az_host_recovery} -U ${az_user_recovery} -d postgres -c "select name from "admin_users" where name = 'root'""
echo " query to recovery db is $az_query_recovery"
az_query="psql -h ${az_host} -U ${az_user} -d postgres -c "select name from "admin_users" where name = 'root'""
